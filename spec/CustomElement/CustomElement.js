...
var upgradeElements = function(inTree, inDefinition) {
  ...
  for (var i=0; element=elements[i]; i++) {
    ...
    if (inDefinition.lifecycle.created) {
      // FIXME: extract key from ancestor
...
var validateArguments = function(inName, inOptions) {
  ...
  if (!inName) {
    // 7.1.1. If NAME is an invalid custom element name,
...
var register = function(inName, inOptions) {
  ...
  // 7.1.6: For DOCUMENT tree and every shadow DOM subtree enclosed by DOCUMENT
  // tree:
  ...
  // 7.1.7. Run custom element constructor generation algorithm with 
  // PROTOTYPE and TEMPLATE as arguments
